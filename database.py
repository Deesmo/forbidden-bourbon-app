...
def get_dashboard_stats():
    ...
    stats = {
        'total_posts': _fetchone(conn, 'SELECT COUNT(*) as cnt FROM posts')['cnt'],
        'drafts': _fetchone(conn, "SELECT COUNT(*) as cnt FROM posts WHERE status = 'draft'")['cnt'],
        'scheduled': _fetchone(conn, "SELECT COUNT(*) as cnt FROM posts WHERE status = 'scheduled'")['cnt'],
        'published': _fetchone(conn, "SELECT COUNT(*) as cnt FROM posts WHERE status = 'published'")['cnt'],
    ...
    stats['published_this_week'] = _fetchone(conn,
