...
def api_generate_image():
    """Generate an image using OpenAI gpt-image-1 with real bottle reference photo"""
    try:
        data = request.get_json()
        ...
        # Map DALL-E quality to gpt-image-1 quality
        quality_map = {'hd': 'high', 'standard': 'medium', 'high': 'high', 'medium': 'medium', 'low': 'low'}
        gpt_quality = quality_map.get(quality, 'high')
        
        # Map DALL-E sizes to gpt-image-1 sizes
        size_map = {'1024x1024': '1024x1024', '1024x1792': '1024x1536', '1792x1024': '1536x1024'}
        gpt_size = size_map.get(size, '1024x1024')
        
        if use_reference:
            # === GPT-IMAGE-1 with real bottle photo as reference ===
            bottle_path = os.path.join(app.static_folder, 'photos', 'bottle-top.jpg')
            if not os.path.exists(bottle_path):
                return jsonify({'success': False, 'error': 'Bottle reference photo not found at static/photos/bottle-top.jpg'}), 400
            ...
            with open(bottle_path, 'rb') as img_file:
                resp = req.post(
                    ...
                    files={'image[]': ('forbidden-bottle.jpg', img_file, 'image/jpeg')},
                    data={
                        'model': 'gpt-image-1',
                        'prompt': prompt,
                        'n': 1,
            ...
            # === GPT-IMAGE-1 text-only (no reference) ===
            resp = req.post(
        ...
        if resp.status_code == 200:
            ...
            # gpt-image-1 returns b64_json by default (no URL)
            if 'b64_json' in image_data and image_data['b64_json']:
                # Save b64 to a file and serve it
                img_bytes = b64.b64decode(image_data['b64_json'])
                filename = f"ai-gen-{int(time.time())}.png"
                ...
                return jsonify({'success': False, 'error': 'No image data returned'}), 500
            ...
            error_msg = resp.json().get('error', {}).get('message', resp.text) if resp.headers.get('content-type','').startswith('application/json') else resp.text
            return jsonify({'success': False, 'error': f'Image generation failed: {error_msg}'}), 500
            ...
            if platform == 'medium':
                ...
                if token:
                    ...
                    if user_resp.status_code == 200:
                        ...
                        pub_resp = req.post(f'https://api.medium.com/v1/users/{user_id}/posts',
                            ...
                            json={'title': article_data.get('title', ''), 'contentFormat': 'html',
                                  'content': article_data.get('content', ''),
                ...
                if wp_site and wp_token:
                    pub_resp = req.post(f'https://public-api.wordpress.com/rest/v1.1/sites/{wp_site}/posts/new',
                        ...
                        json={'title': article_data.get('title', ''), 'content': article_data.get('content', ''),
                ...
                if blog_id and blogger_token:
                    pub_resp = req.post(f'https://www.googleapis.com/blogger/v3/blogs/{blog_id}/posts/',
                        ...
                        json={'kind': 'blogger#post', 'title': article_data.get('title', ''),
                              'content': article_data.get('content', ''),
...
def api_blog_publish(article_id):
    ...
    try:
        ...
        if platform == 'medium':
            ...
            if not token:
                return jsonify({'success': False, 'error': 'Medium integration token required. Set MEDIUM_TOKEN in Render env vars.'}), 400
            ...
            blogger_token = data.get('token', os.environ.get('BLOGGER_TOKEN', ''))
            if not blog_id or not blogger_token:
                return jsonify({'success': False, 'error': 'Blogger blog ID and token required. Set BLOGGER_BLOG_ID and BLOGGER_TOKEN in Render env vars.'}), 400
